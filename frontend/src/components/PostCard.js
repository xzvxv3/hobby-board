...
export default function PostCard({ post, index, isEmpty }) {
    if (isEmpty) {
        return (
            <tr className="h-13.25">
    ...
    return (
        <tr className="group hover:bg-zinc-50 dark:hover:bg-zinc-800/40">
            ...
            <td className="py-4 pl-4 pr-3 text-sm font-medium text-zinc-900 dark:text-zinc-100 sm:pl-6">
                <Link href={`/${post.id}`} className="hover:text-emerald-600 dark:hover:text-emerald-400">
                    {post.title}
                </Link>
            </td>
            ...
            <td className="whitespace-nowrap py-4 pl-3 pr-6 text-right text-sm">
                <div className="flex items-center justify-end gap-1">
                    ...
                    {/* 삭제 버튼 */}
                    <button
                        onClick={() => alert(`${post.id}번 글을 삭제하시겠습니까?`)}
